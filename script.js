...
function initIntro() {
  ...
  const INTRO_MOBILE_BREAKPOINT = 768; // px — intro is skipped on viewports at or below this width
  ...
  // Skip on mobile devices / narrow viewports
  if (window.innerWidth <= INTRO_MOBILE_BREAKPOINT) {
    overlay.classList.add('is-hidden');
    return;
  }
...
function initNav() {
  ...
  const panel      = document.getElementById('navPanel');   // fixed overlay on mobile
  const links      = document.getElementById('navLinks');   // <ul> inside the panel
  ...
  if (toggle && links) {
    ...
    toggle.addEventListener('click', () => {
      panel && panel.classList.contains('is-open') ? closeMenu() : openMenu();
    });
